...
- Forks raise/lower in all eight headings using four-pose studies. Parked vehicles retain their real fork height with matching empty-seat sheets, including a raised carried load. Loaded low/mid/high captures now calibrate the pallet over the blades in all eight headings; the carried ID appears in the fork-height badge instead of masking the tines. Exiting the cab avoids the fork/load area and remains within immediate remount range when the first clear side position is available. Rear-facing cargo is drawn behind the vehicle. Visible stepping, edge fringe and detailed mast/cargo occlusion still need production polish.
