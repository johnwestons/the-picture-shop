...
- Continuous lift candidates now run for east/west, southeast/southwest and northeast/northwest in development-art mode: fixed bodies and moving carriages replace four-step full-body changes in those headings. Parked views use empty-seat bodies; the four-pose sheets remain a guarded fallback. Loaded low/mid/high engine captures were inspected. North and south still step, so forklift art is not yet final.
