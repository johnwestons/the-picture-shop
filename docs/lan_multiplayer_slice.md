...
6. Move a worker into interaction range and press **USE**. The worker waits for the host's decision before a door, client, computer, cutter, skid-wrapper, or pallet-jack action changes authoritative state.
...
- Guest-safe workshop access currently covers the loading-bay door, reception customer quote/decline actions, office-computer inspection and pickup request, Polar cutter production/safety controls, skid-wrapper selection/start request, read-only pallet paperwork, and pallet-jack transport.
