...
Use this checklist for protocol v4's supported LAN direction: any Windows or Android device with a writable selected save can host and own the authoritative shop, while up to three PC or Android devices join as workers. Use the same protocol-compatible game build throughout one pass.
...
10. Try the pallet jack, cutter, windmill, vendor, and delivery truck as a guest. Each must remain host-only and must not open an unsafe guest screen, alter the shop, or create a guest save.
11. Disconnect a worker while it owns a workshop console, then have another worker acquire it. Confirm the stale lease is released and the session continues.
12. Play for 15 minutes, watching for warping, stuck input, stale shop values, duplicate actions, mismatched sound/animation, or a visitor that appears on only one device. End the host session, reload the host's save offline, and verify the final durable state.
...
| A guest force-stops the app while holding a workshop lease | Its worker disappears and its lease is released after the host detects the disconnect; the remaining session continues. |
...
- Build `.8`'s disabled empty **START CYCLE** button and live open-panel pallet refresh pass automated coverage but have not yet received their own physical wrapper-panel pass.
